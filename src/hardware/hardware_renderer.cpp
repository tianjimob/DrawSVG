...
void HardwareRenderer::rasterize_point(float x, float y, Color color) {
    ...
    if (sx < 0 || sx >= context_w)return;
    if (sy < 0 || sy >= context_h)return;
...
void HardwareRenderer::rasterize_line(float x0, float y0,
    ...
    if (sx0 < 0 || sx0 >= context_w)return;
    if (sy0 < 0 || sy0 >= context_h)return;
    if (sx1 < 0 || sx1 >= context_w)return;
    if (sy1 < 0 || sy1 >= context_h)return;
...
void HardwareRenderer::rasterize_triangle(float x0, float y0,
    ...
    if (sx1 < 0 || sx1 >= context_w)return;
    if (sy1 < 0 || sy1 >= context_h)return;
    if (sx2 < 0 || sx2 >= context_w)return;
    if (sy2 < 0 || sy2 >= context_h)return;
