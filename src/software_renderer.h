...
class SoftwareRendererImp : public SoftwareRenderer {
 ...
 private:
#ifdef SSAA
  // task4 for supersample //

  std::vector<unsigned char> supersample_target;

  Color resolve_point(int x, int y);
#elif defined(MLAA)
  // task4 for MLAA //

  bool mlaa = true;
  std::vector<unsigned char> mlaa_sample_target;
#endif

#if defined(SSAA) || defined(MLAA)
  std::vector<unsigned char> render_target_tmp;
#ifdef SSAA
  void clear_my_target() {
      memset(&supersample_target[0], 255, 4 * sample_rate * sample_rate * target_w * target_h);
      memset(&render_target_tmp[0], 255, 4 * target_w * target_h);
  };
#else
  void clear_my_target() {
      memset(&mlaa_sample_target[0], 255, 4 * target_w * target_w);
      memset(&render_target_tmp[0], 255, 4 * target_w * target_h);
  };
#endif
#endif
